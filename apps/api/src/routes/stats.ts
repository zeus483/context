...
export function registerStatsRoutes(app: FastifyInstance) {
  ...
  app.get("/api/stats/profile", async (req) => {
    const { userId, guestId } = req.query as { userId?: string; guestId?: string };
    if (!userId && !guestId) return { games: 0, wins: 0, streak: 0 };
    const entries = await prisma.gamePlayer.findMany({
      where: {
        ...(userId ? { userId } : {}),
        ...(guestId ? { guestId } : {})
    ...
    for (const entry of entries) {
      if (entry.game.winnerSide === entry.role) wins += 1;
