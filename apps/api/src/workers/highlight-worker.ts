...
const worker = new Worker(
  "highlight-render",
  async (job) => {
    const { highlightId } = job.data as { highlightId: string };
    const highlight = await prisma.highlight.findUnique({ where: { id: highlightId } });
    if (!highlight) return;

    const metadata = (highlight.metadata || {}) as Record<string, unknown>;
    const contexts = metadata.contexts as { a?: string; b?: string } | undefined;
    const voteSummary = metadata.votes as Record<string, string> | undefined;
