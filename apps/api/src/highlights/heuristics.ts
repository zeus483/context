...
function scoreText(text: string, keywords: Set<string>) {
  const words = tokenize(text);
  let score = 0;
  for (const w of words) {
    if (keywords.has(w)) score += 1;
  }
  return score;
}

export function pickSuspiciousQuote(runtime: RuntimeData) {
  const contexts = runtime.state.contexts;
  if (!contexts) return null;
  const aKeywords = keywordSet(contexts.a);
  const bKeywords = keywordSet(contexts.b);

  let best: { score: number; quote: string; authorId: string } | null = null;
