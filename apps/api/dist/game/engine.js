...
import { DEFAULT_TIMERS, MODE_ROUNDS_TO_WIN, Phases } from "@cc/shared";
...
export function initRoomState(roomCode, hostId, config, players) {
    return {
        roomCode,
        phase: Phases.LOBBY,
        phaseEndsAt: null,
...
async function broadcastState(io, runtime) {
...
async function emitBrief(io, runtime) {
...
async function emitPrompt(io, runtime, index) {
...
function setPhase(runtime, phase, durationSec) {
    runtime.state.phase = Phases[phase];
    runtime.state.phaseEndsAt = Date.now() + durationSec * 1000;
    runtime.state.phaseNonce = nanoid(6);
}
async function scheduleNext(io, runtime, nextPhase, delaySec, onStart) {
    const timer = setTimeout(async () => {
        setPhase(runtime, nextPhase, runtime.state.config.timers[nextPhase.toLowerCase()] ?? delaySec);
...
export async function startGame(io, runtime) {
...
async function startRound(io, runtime) {
...
export async function advancePhase(io, runtime) {
...
export async function handleSubmit(runtime, playerId, promptId, answerText) {
    if (!runtime.answers[promptId])
        runtime.answers[promptId] = {};
    runtime.answers[promptId][playerId] = answerText;
    await saveRuntime(runtime.state.roomCode, runtime);
}
export async function handleVote(runtime, voterId, targetId) {
    runtime.votes[voterId] = targetId;
    await saveRuntime(runtime.state.roomCode, runtime);
}
function resolveVotes(runtime) {
    const tally = {};
    ...
    if (tied.length > 1 && !runtime.state.config.allowTieElimination)
        return null;
    if (tied.length > 1) {
...
async function handleEndOrNext(io, runtime) {
    const infiltrators = Object.entries(runtime.state.roles || {})
        .filter(([, role]) => role === "INFILTRATOR")
        .map(([id]) => id);
    const aliveInfiltrators = infiltrators.filter((id) => runtime.state.players.find((p) => p.id === id && p.isAlive !== false));
