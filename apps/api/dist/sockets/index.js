...
const publicState = (state) => ({ ...state, roles: undefined, constraints: undefined, contexts: undefined });
export function registerSockets(io) {
    io.on("connection", (socket) => {
        ...
        socket.on("room:join", async (payload) => {
            const { roomCode, nickname, avatar } = payload;
            ...
            if (!runtime) {
                ...
                const configPayload = room.config;
            ...
            if (!member) {
                ...
                runtime.state.players = runtime.state.players.map((p) => p.id === member.id ? { ...p, nickname: cleanNickname, avatar: cleanAvatar, isConnected: true } : p);
            }
            socket.join(roomCode);
            socket.data.roomCode = roomCode;
            socket.data.playerId = member.id;
        ...
        socket.on("room:leave", async () => {
            ...
            if (!runtime)
                return;
            runtime.state.players = runtime.state.players.map((p) => p.id === socket.data.playerId ? { ...p, isConnected: false } : p);
        ...
        socket.on("room:kick", async ({ targetPlayerId }) => {
            ...
            if (!runtime)
                return;
            if (runtime.state.hostId !== socket.data.playerId)
                return;
        ...
        socket.on("game:submit", async ({ promptId, answerText, phaseNonce }) => {
            ...
            if (!runtime)
                return;
            if (runtime.state.phase !== Phases.PROMPT1 && runtime.state.phase !== Phases.PROMPT2)
                return;
            if (!runtime.state.players.find((p) => p.id === socket.data.playerId && p.isAlive !== false))
                return;
            if (phaseNonce && phaseNonce !== runtime.state.phaseNonce)
                return;
        ...
        socket.on("game:discuss:message", async ({ text }) => {
            ...
            if (!runtime)
                return;
            if (runtime.state.phase !== Phases.DISCUSS)
                return;
            if (!runtime.state.players.find((p) => p.id === socket.data.playerId && p.isAlive !== false))
                return;
            const now = Date.now();
        ...
        socket.on("game:vote", async ({ targetPlayerId, phaseNonce }) => {
            ...
            if (!runtime)
                return;
            if (runtime.state.phase !== Phases.VOTE)
                return;
            if (phaseNonce && phaseNonce !== runtime.state.phaseNonce)
                return;
            if (runtime.votes[socket.data.playerId || ""])
                return;
            if (!runtime.state.players.find((p) => p.id === socket.data.playerId && p.isAlive !== false))
                return;
            if (!runtime.state.players.find((p) => p.id === targetPlayerId))
                return;
        ...
        socket.on("stream:join", async ({ roomCode }) => {
            ...
            if (!runtime)
                return;
            if (!runtime.state.config.streamMode)
                return;
